...
if ($data[19]) {
    ...
    if (!$c_status[19]) {
        if ($data[19] < 10) {
            ...
            if ($town[12] + $town[3] + $upk[$data[19]] > $lim[3]) {
                label("</br>".$lang['noHouses']);
            }

            if (!(($res[0] >= $cost[0] * pow($r, $data[19])) && ($res[1] >= $cost[1] * pow($r, $data[19])) && ($res[2] >= $cost[2] * pow($r, $data[19])) && ($res[3] >= $cost[3] * pow($r, $data[19])) && ($res[4] >= $cost[4] * pow($r, $data[19])))) {
                label("</br>".$lang['noResources']);
            }

            echo "</br>------------------------------------------</br>";
        } else {
            echo $lang['buildingMaxLvl'];
        }
    } else {
        echo $lang['beingUpgraded'];
    }
} else {
    echo $lang['constrBuilding'];
}
?>
...
<?php
if ($data[19]) {
 $dur = explode(":", $weapons[9][4]);
 $cost = explode("-", $weapons[9][3]);
